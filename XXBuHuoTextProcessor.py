...
class XXBuHuoTextProcessor:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "展开参数": ("BOOLEAN", {"default": False, "label_on": "收起参数", "label_off": "展开参数"}),
                "输出模式": ("BOOLEAN", {
                    "default": False,
                    "label_on": "输出分段列表",
                    "label_off": "输出原始文本",
                    "tooltip": "控制端口输出的内容：\n● 输出原始文本（忽略所有的分段方式、选取和筛选逻辑）。\n● 输出分段列表（输出分割处理后的内容，按分段方式进行分割）。"
                }),
                "段落优化": ("BOOLEAN", {
                    "default": True,
                    "label_on": "去除首尾空格",
                ...
                "选取段落": ("STRING", {
                    "default": "0",
                    "placeholder": "输入要选取的段落，用逗号分隔，如1,4,6",
                    "tooltip": "【分割后段落选取】\n决定选取哪些段落输出。\n● 输入 0 或留空：输出所有段落。\n● 输入 1,3,5：只输出第1、3、5段，丢弃其他。\n此设置会改变[总段]和[段落x]端口的内容。"
                }),
                "筛选段落": ("INT", {
                    "default": 0,
                    "min": 0,
    ...
    MAX_OUTPUTS = 200
    RETURN_TYPES = ("STRING", "STRING") + ("STRING",) * MAX_OUTPUTS
    ...
    def split_paragraphs(self, text, 展开参数, 分段方式, 段落优化, 输出模式, 输出段落, 选取段落, 筛选段落, 输入端口,
                         **kwargs):
        input_count = 输入端口
        collected_texts = []
        ...
        if not text:
            return {"ui": {"text": [""]}, "result": ("0", "",) + ("",) * 200}
        if not 输出模式:
            final_text = text.strip() if 段落优化 else text
            cnt = "1"
            to = final_text
            paras = [final_text]
        else:
            if 分段方式 == "端口":
                ...
                for m in raw_paras:
                    ...
                    if pl:
                        paras.append(pl)
            cnt, sp, isa = str(len(paras)), [], 选取段落.strip() in ("0", "")
            if not isa:
                si = re.split(r'[。,，./\\]', 选取段落)
                for i in si:
                    try:
                        idx = int(i.strip()) - 1
                        if 0 <= idx < len(paras): sp.append(paras[idx])
                    except:
                        continue
            else:
                sp = paras.copy()
            fs = paras if isa else sp
            fs_len = len(fs)
            if 筛选段落 > 0 and 筛选段落 > fs_len: raise ValueError(f"筛选段落({筛选段落})超过实际可用段落数({fs_len})")
            if 筛选段落 == 0:
                to = fs
            else:
                to = fs[筛选段落 - 1] if 筛选段落 - 1 < len(fs) else ""
        max_out = self.MAX_OUTPUTS
        po = [paras[i] if i < 输出段落 and i < len(paras) else "" for i in range(max_out)]
        return {"ui": {"text": [text]}, "result": (cnt, to,) + tuple(po)}


NODE_CLASS_MAPPINGS = {"XXBuHuoTextProcessor": XXBuHuoTextProcessor}
NODE_DISPLAY_NAME_MAPPINGS = {"XXBuHuoTextProcessor": "XXBuHuo 多功能文本处理"}
